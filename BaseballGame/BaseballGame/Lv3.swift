...
//    }
    ...
    /* 조건
     1. 입력값이 세글자인지 확인
     2. 숫자가 아닌 값 확인 (백의 자리가 0인 String 값은 Int로 변환하면서 없어짐)
        Ex) "048" -> 48
     3. Int 값을 다시 String 배열로 변환하여 Set을 통해 중복값 삭제
     4. 필터링된 입력값이 3글자인지 확인
    */
    func validationInput(input: String) -> Bool {
        guard input.count == 3, let intInput = Int(input) else { return false } // 1, 2.
        let filteredInput = Set(String(intInput).map{$0}) // 3.
        
        if filteredInput.count == 3 {   // 4.
            return true
